...
class DeleteReferenceHomepage extends Component{
    constructor(props){
        super(props);
        this.state = {
            ReferenceHomepage: [],
    ...
    deleteReferenceHomepage = () => {

        if(this.props.ReferenceHomepageToDelete.length > 0){
            const options = {
                method:'DELETE',
                headers: new Headers({
            ...
            let url = `${REACT_APP_SERVER_ADDRESS_FULL}/api/image/${this.props.ReferenceHomepageToDelete[1]}`;
            console.log("URL : ",url)
            fetch(url, options).then(res => res.json()).then(res => console.log(res)).catch(err => console.log("ERROR : ",err));
            
    
            url = `${REACT_APP_SERVER_ADDRESS_FULL}/api/homepage/${this.props.ReferenceHomepageToDelete[0]}`;
    ...
    render(){
        const { ReferenceHomepageToDelete } = this.props;
        ...
        return(
            <div>
                ...
                <div className="modal-footer">
                    {this.state.checkBox && this.props.ReferenceHomepageToDelete.length > 0
                        ?
                        <button type="button" class="btn btn-primary" data-dismiss="modal" onClick={this.deleteReferenceHomepage}>Oui</button>
                        :
                        <button type="button" class="btn btn-secondary">Oui</button>
                        }
                        <button type="button" class="btn btn-danger" data-dismiss="modal" onClick={this.cancelDeleteReferenceHomepage}>Annuler</button>
                </div>
...
export default DeleteReferenceHomepage;