...
router.get("/", (req, res) => {
  const sql = `SELECT h.id, h.title, h.subtitle, h.description, i.name, i.url, i.alt, i.homepage_id, l.name AS language, l.locale FROM homepage AS h JOIN image AS i ON h.image_id = i.homepage_id JOIN language AS l ON h.language = l.id WHERE h.section=? && i.section=? && l.locale=?`;
